#!/usr/bin/python3
#Hice este guión porque un amigo me lo pidió :P
...
def init():
  ...
  try:
         while (conti > 0):
           ...
           print(f'{C}[{G}i{C} Email enviado exitosamente!')
           conti=conti-1
         print(f'{C}[{G}i{C}] Hecho! Espere la respuesta de soporte.')
           ##############################
  except Exception as erro:
    print(f"{error} Compruebe si la opción 'Aplicaciones menos seguras' se ha habilitado o si ingresó el correo electrónico / contraseña correctamente.\n{warning}: "+str(erro));time.sleep(5)
Sair = False
while(Sair == False):
    try:
